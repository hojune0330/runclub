...
## TL;DR — 현지 매니저 작업은 이것만 지키면 됩니다

1. 장부는 raw 데이터(이름·결제일·개월·금액)만 코드에 넣고, 시작/종료일은 규칙 함수가 계산합니다.
2. 적용 전에는 항상 dry-run으로 `발급 가능 / 회원 없음 / 동명이인 / 이미 발급`을 먼저 봅니다.
3. 실제 발급은 트랜잭션으로 처리하고, 수강권과 지급 기록을 함께 남깁니다.
4. 관리자 화면은 목록에서 핵심만 보여주고, 행을 눌러 세부 정보를 확인하는 흐름을 기본으로 합니다.
5. 검증은 `npm run passes:verify-spring`, 타입체크, 대상 ESLint까지 확인합니다.
...
| 일괄 import | `scripts/import-spring-2026-passes.mjs` | "dry-run 기본 · 이름 매칭 · 멱등 · 트랜잭션 · 지급 기록 통합" |
...
- **지급 기록 통합**: 운영 장부 일괄 발급도 `pass_grant_records` 에 남겨 관리자 화면의 지급 기록에서 함께 본다.
...
3. **지급 기록 리포트** — `pass_grant_records` 기준 기간별 매출/개강대기 분리 집계.
...
| 만료 임박/입금 요약 | `src/lib/pass-reports.ts` | 만료 D-day·기간별 매출·개강대기 분리 |
