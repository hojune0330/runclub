 ...
 * 반환: { ok, imported, duplicate, mileageEarned, truncated, skipped }
 *
...
export async function POST(req: NextRequest) {
  ...
  if (classId) {
    const enrolled = await dbGet<any>(
      `SELECT 1 FROM class_enrollments WHERE class_id = $1 AND member_id = $2`,
  ...
  try {
    ...
    return NextResponse.json({ error: e?.message || '파일을 해석하지 못했습니다' }, { status: 422 });
  }

  if (parsed.activities.length === 0) {
    return NextResponse.json({
      ok: true, imported: 0, duplicate: 0, mileageEarned: 0,
  ...
  for (const a of parsed.activities) {
    ...
    const dup = await dbGet<any>(
      `SELECT 1 FROM activity_logs WHERE member_id = $1 AND source = $2 AND source_ref = $3`,
      [auth.memberId, provider, sourceRef]
    );
  ...
  return NextResponse.json({
    ok: true, imported, duplicate, mileageEarned,
    truncated: parsed.truncated, skipped: parsed.skipped,
  });
}
